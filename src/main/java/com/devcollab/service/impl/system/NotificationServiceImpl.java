...
public class NotificationServiceImpl implements NotificationService {
    ...
    public void notifyJoinRequestRejected(Project project, User requester, String reviewerEmail) {
        ...
        createNotification(requester, "JOIN_REQUEST_REJECTED", project.getProjectId(),
                ...
                "PM (" + reviewerEmail + ") đã từ chối yêu cầu tham gia dự án \""
                        + project.getName() + "\".",
                "/view/pm/project/board?projectId=" + project.getProjectId(),
                project.getCreatedBy());
    ...
    public void notifyTaskEvent(Task task, User actor, String eventType, String message,
        ...
        try {
            ...
            String link = "/view/pm/project/board?projectId=" + project.getProjectId() + "&taskId="
                    + task.getTaskId();

            if (specificReceiver != null) {
                String msg = buildTaskMessage(eventType, managed.getTitle(), message, true);
            ...
            for (User receiver : receivers) {
                String msg = buildTaskMessage(eventType, managed.getTitle(), message, false);
    ...
    private String buildTaskMessage(String eventType, String taskTitle, String custom,
            boolean direct) {
        return switch (eventType) {
            case "TASK_MEMBER_ADDED" -> direct ? "Đã thêm bạn vào công việc \"" + taskTitle + "\""
                    : "Đã thêm thành viên vào \"" + taskTitle + "\"";
            case "TASK_MEMBER_REMOVED" -> direct ? "Đã xóa bạn khỏi công việc \"" + taskTitle + "\""
                    : "Đã xóa một thành viên khỏi \"" + taskTitle + "\"";
            case "TASK_COMMENTED" -> "Đã bình luận: \"" + custom + "\" trong \"" + taskTitle + "\"";
            case "TASK_DUE_SOON" -> "⏰ Công việc \"" + taskTitle + "\" sắp đến hạn!";
            case "TASK_FOLLOWED" -> "Đang theo dõi công việc \"" + taskTitle + "\"";
    ...
    public void notifyMentions(Task task, User actor, List<CommentDTO> mentions) {
        ...
        try {
            ...
            String taskLink = "/view/pm/project/board?projectId=" + project.getProjectId()
    ...
    public void notifyChangeProfile(User user) {
        ...
        createNotification(user, "PROFILE_UPDATED", user.getUserId(), "Cập nhật hồ sơ",
                "Thông tin tài khoản của bạn đã được thay đổi.", "/user/view/profile", user);
    }
    ...
    public void notifyChangePassword(User user) {
        ...
        createNotification(user, "PASSWORD_CHANGED", user.getUserId(), "Đổi mật khẩu",
                "Mật khẩu tài khoản của bạn đã được cập nhật thành công.", "/user/view/profile",
                user);
    }
    ...
    public void notifyPaymentSuccess(User user, PaymentOrder order) {
        ...
        log.info("📢 Đã tạo thông báo thanh toán thành công cho {}", user.getEmail());
    }
    ...
    public List<Map<String, Object>> findRecentByProject(Long projectId) {
        try {
            ...
            log.error("❌ Error fetching notifications for project {}: {}", projectId,
                    e.getMessage());
            return List.of();
        }
    }
    ...
    private String determinePriority(String type) {
        ...
        return switch (type.toUpperCase()) {
            case "TASK_COMMENT_MENTION", "PROJECT_COMMENT_MENTION", "MEMBER_ADDED", "TASK_MEMBER_ADDED", "PROJECT_MEMBER_ROLE_UPDATED", "PASSWORD_CHANGED", "PAYMENT_SUCCESS", "PROJECT_LINK_REGENERATED", "TASK_DUE_SOON", "JOIN_REQUEST_RECEIVED", "SCHEDULE_CREATED" -> "HIGH";
    ...
    private static final Map<String, String> TITLE_MAP =
            Map.ofEntries(Map.entry("PROJECT_CREATED", "Dự án mới"),
                    Map.entry("SCHEDULE_CREATED", "Lịch họp mới"),
    ...
    private static final Map<String, String> ICON_MAP = Map.ofEntries(
            ...
            Map.entry("SCHEDULE_CREATED", "📅"), Map.entry("MEMBER_ADDED", "👥"),
            Map.entry("TASK_MEMBER_ADDED", "👤"), Map.entry("TASK_COMMENTED", "💬"),
            Map.entry("TASK_COMMENT_MENTION", "📣"), Map.entry("PROJECT_COMMENT_MENTION", "📢"),
            Map.entry("TASK_DUE_SOON", "⏰"), Map.entry("TASK_FOLLOWED", "⭐"),
            Map.entry("PROJECT_LINK_REGENERATED", "🔗"), Map.entry("JOIN_REQUEST_RECEIVED", "📩"),
            Map.entry("JOIN_REQUEST_APPROVED", "✅"), Map.entry("JOIN_REQUEST_REJECTED", "❌"),
            Map.entry("PASSWORD_CHANGED", "🔑"), Map.entry("PROFILE_UPDATED", "⚙️"));
    ...
    @Override
    @Transactional
    public void notifyScheduleCreated(Project project, User creator, ProjectSchedule schedule) {
        if (project == null || schedule == null) {
            log.warn(
                    "⚠️ [Notification] Project or Schedule is null for schedule creation notification");
            return;
        }

        try {
            // Get all project members except the creator
            List<User> projectMembers = project.getMembers().stream().map(ProjectMember::getUser)
                    .filter(Objects::nonNull)
                    .filter(user -> creator == null
            ...
            // Use project ID as reference if schedule ID is not available yet
            Long referenceId =
                    (schedule.getId() != null) ? schedule.getId() : project.getProjectId();

            String link = "/view/pm/project/schedule/calendar?projectId=" + project.getProjectId();

            String message =
                    "Đã tạo lịch họp \"" + scheduleTitle + "\" trong dự án \"" + projectName + "\"";

            // Send notification to all project members
            for (User member : projectMembers) {
                createNotification(member, "SCHEDULE_CREATED", referenceId, "Lịch họp mới", message,
                        link, creator);
            }

            log.info("📅 [Notification] Sent SCHEDULE_CREATED to {} members in project {}",
                    projectMembers.size(), project.getName());

        } catch (Exception e) {
            log.error("❌ [Notification] Failed to create schedule notifications: {}",
                    e.getMessage(), e);
        }
    }
