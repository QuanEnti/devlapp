    ...
    public class NotificationServiceImpl implements NotificationService {
        ...
        public void notifyScheduleCreated(Project project, User creator, ProjectSchedule schedule) {
            if (project == null || schedule == null) {
                log.warn("⚠️ [Notification] Project or Schedule is null for schedule creation notification");
                return;
            }

            try {
                // Get all project members except the creator
                List<User> projectMembers = project.getMembers().stream()
                        .map(ProjectMember::getUser)
                        .filter(Objects::nonNull)
                        .filter(user -> creator == null || !user.getUserId().equals(creator.getUserId()))
                ...
                Long referenceId = (schedule.getId() != null) ? schedule.getId() : project.getProjectId();

                String link = "/view/pm/project/schedule/calendar?projectId=" + project.getProjectId();

                String creatorName = (creator != null) ? creator.getName() : "Hệ thống";
                String message = creatorName + " đã tạo lịch họp \"" + scheduleTitle + "\" trong dự án \"" + projectName + "\"";

                // Send notification to all project members
                for (User member : projectMembers) {
                    createNotification(member, "SCHEDULE_CREATED", referenceId,
                            "Lịch họp mới",
                            message,
                            link, creator);
                }
                ...
                log.error("❌ [Notification] Failed to create schedule notifications: {}", e.getMessage(), e);
            }
        }
